...
<template>
  ...
  />
  <CalendarComponent class="grid" :events="events" :date="date" />
  <EventsListComponent class="events" :events="events" :date="date" />
</template>
  ...
  setup() {
    ...
    const transformEventsDates = (collection) => {
      return collection
        ...
        .map((event) => ({
          ...event,
          start: Date.parse(event.start) ? new Date(event.start) : event.start,
          end: Date.parse(event.end) ? new Date(event.end) : event.end,
        }))
    }
    ...
    const loadCyberEvents = async (start, end) => {
      ...
      return Object.values(utils.groupByDateMonth(transformEventsDates(data)))
        .map((events, index) => {
          return events.map((event) => ({
            ...event,
            class: CSS_COLOR_CLASS[index % CSS_COLOR_CLASS.length],
          }))
        })
        .flatMap((event) => event)
    }
    ...
    const onListChange = async (dates) => {
      if (!isSameDate(date.value, dates.startDate)) {
        ...
        events.value = await loadCyberEvents(
          new Date(date.value.getFullYear(), 0, 1, 0, 0, 0, 0).toISOString(),
          new Date(
